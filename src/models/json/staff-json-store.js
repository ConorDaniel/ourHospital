...
export const staffJsonStore = {
  async getAllStaffs() {
    await db.read();
    return db.data.staff || []; // ✅ Ensure staff array exists
  },
  ...
  async addStaff(departmentId, staff) {
    await db.read();
    if (!db.data.staff) db.data.staff = []; // ✅ Fix: Always initialize staff
    staff._id = v4();
  ...
  async getStaffByDepartmentId(id) {
    await db.read();
    return (db.data.staff || []).filter((staff) => staff.departmentid === id); // ✅ Fix: Prevent undefined errors
  },
  ...
  async deleteStaff(id) {
    await db.read();
    if (!db.data.staff) return; // ✅ Fix: Prevent deleting from undefined
